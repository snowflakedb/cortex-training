...
- **Recipe mode** — run `python -m recipes.<name>.train config=...` for an
end-to-end workflow: job creation, data loading, training loop, checkpointing,
and cleanup. Each recipe's README documents its config knobs and expected
results. This is the recommended path for most users.
- **CLI mode** — use individual commands (`cortex-training submit`,  
`cortex-training fwd-bwd`, `cortex-training step`, etc.) to manually control  
each step. Recipes are built from these same primitives. This mode is useful  
for debugging, experimentation, or building custom training loops.

## Choose a Path

- **New users (required, do this first):** follow the [getting-started guide](getting-started/README.md)
  to install the client, configure a connection, and run a short SFT job.
- **Training users:** follow a step-by-step workflow for
  [full-parameter SFT](guides/workflows/sft-full-parameter.md),
  [LoRA SFT](guides/workflows/sft-lora.md), or
  [GRPO reinforcement learning](guides/workflows/grpo-rl.md).
- **Inference users:** follow the
  [inference serving workflow](guides/workflows/inference-serving.md) to serve
  and query a model endpoint.
...
| Page                                              | Description                                                   |
| ------------------------------------------------- | ------------------------------------------------------------- |
| [Prerequisites](getting-started/prerequisites.md) | Account access, authentication, and GPU capacity requirements |
| [Setup](getting-started/setup.md)                 | Install the client and create a connection config             |
